...
# iarchv7.sh — Arch Linux base en Hyper-V Gen2 (UEFI), booteable con systemd-boot.
# - Particiona (ESP + ROOT), instala base, configura red (DHCP), systemd-boot + fallback.
# - Corrige el problema de ln/resolv.conf para que no aborte con set -e.

# ======= Parámetros (puedes exportarlos antes de ejecutar) =======
...
NO_REBOOT="${NO_REBOOT:-}"     # si es no-vacío, no reinicia al final
...
# (Opcional) archivo packages.x86_64 junto al script
...
# Red: systemd-networkd DHCP para e* y systemd-resolved
...
# Link resolv.conf (sin romper si ya apunta al stub)
...
# Fallback UEFI obligatorio para Hyper-V (BOOTX64.EFI)
