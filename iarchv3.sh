...
# arch-install-uefi-strict.sh (network check fixed)
# Hyper-V Gen2 (UEFI, Secure Boot OFF). Falla en errores criticos.
...
net_ok() {
  # Prefer HTTP(S) over ICMP; no cambios en ISO.
  if command -v curl >/dev/null 2>&1; then
    curl -fsSIL --max-time 8 https://archlinux.org/ >/dev/null 2>&1 && return 0
  fi
  if command -v wget >/dev/null 2>&1; then
    wget -q --spider --timeout=8 https://archlinux.org/ && return 0
  fi
  # Fallback: abrir TCP 443 (sin depender de curl/wget/ping)
  timeout 6 bash -c '>/dev/tcp/archlinux.org/443' 2>/dev/null && return 0
  return 1
}

# ---------- Preflight ----------
echo "== PRE-FLIGHT =="
[ -d /sys/firmware/efi ] || die "UEFI not detected. Use Hyper-V Gen2 and disable Secure Boot."
...
net_ok || die "No network connectivity detected (curl/wget/TCP). Run snet.sh and retry."
ok "Network ready"
...
findmnt -no FSTYPE,TARGET /mnt/boot | grep -qi "vfat.*\/mnt\/boot" || die "/mnt/boot is not the FAT32 ESP"
...
[ -f /mnt/boot/EFI/systemd/systemd-bootx64.efi ] || die "systemd-bootx64.efi missing in ESP"
