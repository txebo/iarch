...
# One-shot Arch installer for Hyper-V Gen2 (UEFI, Secure Boot OFF).
# Fails fast on any critical issue. Installs systemd-boot and enforces UEFI fallback.
...
# Optional extras file next to this script (one package per line, '#' comments allowed)
...
net_ok || die "No network connectivity (HTTP/HTTPS/TCP). Run your network bootstrap first."
...
# Optional extras from packages.x86_64 (validate names first)
EXTRA_PKGS=()
if [ -f "$PKG_LIST" ]; then
  ...
  if [ "${#EXTRA_PKGS[@]}" -gt 0 ]; then
    ...
    if [ "${#BAD[@]}" -gt 0 ]; then
      die "invalid packages in packages.x86_64: ${BAD[*]}"
    fi
...
# --------------- UEFI fallback (critical for Hyper-V) ---------------
echo "== UEFI FALLBACK =="
