#!/usr/bin/env sh
set -eu
# net-68-bkend.sh — red fija + SMB para BKEND 192.168.68.x
...
ip addr add 192.168.68.21/24 dev "$IFACE"
ip route replace default via 192.168.68.1 dev "$IFACE"
printf "nameserver 1.1.1.1\nnameserver 8.8.8.8\n" >/etc/resolv.conf

# Pruebas rapidas (no fatales)
ping -c1 -W1 192.168.68.1 >/dev/null 2>&1 || echo "[WARN] GW no responde"
ping -c1 -W1 192.168.68.128 >/dev/null 2>&1 || echo "[WARN] Host SMB no responde"
...
FSTAB_LINE="//192.168.68.128/shared /mnt/shared cifs credentials=/etc/cifs-creds/Neural-TXIA_shared.cred,vers=3.1.1,sec=ntlmssp,uid=$(id -u),gid=$(id -g),iocharset=utf8,file_mode=0644,dir_mode=0755,x-systemd.automount,_netdev,noauto 0 0"
grep -Fq "$FSTAB_LINE" /etc/fstab || echo "$FSTAB_LINE" >> /etc/fstab
systemctl daemon-reload || true
systemctl restart remote-fs.target || true

echo "[OK] Listo. NIC=$IFACE  IP=192.168.68.21/24  GW=192.168.68.1  SMB=//192.168.68.128/shared -> /mnt/shared"
